...
export function createProxyRoutes(configOverrides?: { opencodeUrl?: string }) {
  ...
  routes.get("/api/session", async (c) => {
    ...
    if (requestedWorkspace && requestedWorkspace !== userId) {
  ...
  routes.post("/api/session/:id/abort", (c) => {
    const userId = c.get("userId") as string
    const directory = c.get("directory") as string
    return proxyRequest(config, c.req.url, "POST", c.req.raw.headers, null, directory, userId)
  })
  ...
  routes.get("/api/session/:id/context", (c) => {
    const userId = c.get("userId") as string
    const directory = c.get("directory") as string
    return proxyRequest(config, c.req.url, "GET", c.req.raw.headers, null, directory, userId)
  })
  ...
  routes.patch("/api/session/:id", (c) => {
    const userId = c.get("userId") as string
    return proxyRequest(config, c.req.url, "PATCH", c.req.raw.headers, c.req.raw.body, undefined, userId)
  })

  routes.delete("/api/session/:id", (c) => {
    const userId = c.get("userId") as string
    return proxyRequest(config, c.req.url, "DELETE", c.req.raw.headers, null, undefined, userId)
  })
