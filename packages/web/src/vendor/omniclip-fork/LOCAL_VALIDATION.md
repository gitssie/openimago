...
| 12 | **Filmstrip = one 9:16 first-frame thumbnail per second** (openimago-ugli/u3qq → **openimago-7vrd final**) | Open 时间线; inspect a clip's `.filmstrip` → N `.sprite-cell` (1 per second), each containing one 28px-wide `.sprite-thumb` | FINAL layout (supersedes row 10's px5g mapping, the ugli percentage-stretch, AND the u3qq dense 28px tiling). The strip is a "which video is this" marker: EXACTLY ONE thumbnail of the video's FIRST frame (frame 0) per second of the clip, at NATIVE 9:16 aspect — NO time→frame mapping, NO per-cell second labels. `cellCount = ceil(clipDurationSeconds)` (= `filmstrip_duration_seconds`), clamped to MAX_CELLS; each `.sprite-cell` is `flex:1 1 0` so the N cells split the effect's real rendered width equally (one cell == one second at true scale — no 2^zoom math). Inside each cell a FIXED `.sprite-thumb` box `FILMSTRIP_TILE_W×CELL_H` = 28×50 is LEFT-aligned at the second's start, showing frame 0 cropped by PERCENTAGE (`spriteBackgroundSizeX` = `frameCount*100%`, `background-position-x:0`); box and frame are both 9:16 → NO distortion (the ugli/u3qq versions stretched the frame across a wide cell → horizontal bar). The remaining cell width shows the lane background — GAPS between thumbnails are intended (28px thumb vs ~125px/sec cell). Confirm: exactly one crisp 9:16 thumbnail per second (s02 15s clip → 15, s07 64.76s → 65), all the same first frame within a clip, left-aligned with gaps; different-video clips show different first frames; clip<1s shows ≥1; NO sprited clip renders 0. Top SECONDS come from omniclip's own `TimeRuler` (`views/time-ruler`, unpatched; same `2^zoom` scale + `timeline` scroll origin) — we add NO duplicate labels; `applyImagoTheme` only sets `--imago-*` color tokens, it does not hide/move the ruler. The dead `.filmstrip img.thumbnail` rule remains removed; the u3qq `effectWidthPx`/`filmstripTileCount`/`state.zoom` read was removed as dead (one-per-second needs no pixel width). |
