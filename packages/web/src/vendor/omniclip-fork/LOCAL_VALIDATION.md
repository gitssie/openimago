...
| 12 | **Filmstrip = first-frame marker, one cell/sec** (openimago-ugli) | Open 时间线; inspect a clip's `.filmstrip` `.sprite-cell` divs | SUPERSEDES the px5g per-second source-frame mapping in row 10. The strip is only a "which video is this" marker: every `.sprite-cell` shows the video's FIRST frame (frame 0); there is NO time→frame mapping (no `secondsPerCell`/inPoint/`filmstrip_source_duration_seconds` read in render). Still one cell per second of the clip (`cellCount = ceil(filmstrip_duration_seconds)`) so duration is legible. Each cell crops to exactly ONE frame by PERCENTAGE (`filmstrip-sprite-css`): `background-size-x = frameCount*100%`, `background-position-x = 0` — so a single frame fills the cell whatever its flex pixel width (the earlier pixel `background-size` leaked ~3-4 frames per wide cell). Confirm: every cell in a clip shows the SAME (first) frame; different-video clips show different first frames; NO sprited clip renders 0 cells. The `.filmstrip img.thumbnail` CSS rule was removed (dead — render uses `.sprite-cell` background divs, no `<img>`). |
