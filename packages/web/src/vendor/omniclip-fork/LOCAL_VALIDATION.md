...
| 10 | **Filmstrip 9:16, dense edge-to-edge, not stretched** (openimago-audw / fhnz / 6br9) | Inspect a clip's `.filmstrip` `<img class="thumbnail">` frames (portrait shot videos); scroll the timeline horizontally | Each frame is a 28×50 9:16 CENTER-COVER crop rendered at 28px wide, and frames TILE CONTINUOUSLY edge-to-edge with NO black gaps (a real-NLE strip) — not sparse 28px frames in ~116px slots. Density: `filmstrip.patch.ts` sets the cell count = `effect_width / 28` (was the upstream real-frame-count → ~116px cells) so `#width_of_frame` ≈ 28 and the visible-window draw steps by `#width_of_frame` (was `/100`). Frames are still un-stretched (`effect-styles.patch.ts` pins `.thumbnail { width:28px !important; object-fit:cover }`). Horizontal scroll keeps frames aligned. Confirm: adjacent `<img>`s butt together (translateX deltas ≈ 28px), no gap. |
