...
| 9 | **Clip filmstrips** (openimago-vwjl) | After assemble/hydrate, look at each clip at rest (no playback) | Every clip's `.filmstrip` shows video frame thumbnails (`<img class="thumbnail">` with a data-URL `src`), NOT an empty solid block. Root cause was `media.get_file()` hanging on `#files_ready` (never set by the programmatic import); fixed by `get_imported_files()` in import-from-url.ts + a post-mount `on_media_change("added")` re-publish in hydrate-from-cut.ts |
