...
| 10 | **Filmstrip 9:16, dense edge-to-edge, not stretched** (openimago-audw / fhnz / 6br9) | Inspect a clip's `.filmstrip` `<img class="thumbnail">` frames (portrait shot videos); scroll the timeline horizontally | Each frame is a 28×50 9:16 CENTER-COVER crop rendered at 28px wide, and frames TILE CONTINUOUSLY edge-to-edge with NO black gaps (a real-NLE strip) — not sparse 28px frames in ~116px slots. Density: `filmstrip.patch.ts` exposes `frames_count` as a LIVE GETTER = `clamp(ceil(effect_width/28),1,600)` (NOT frozen at init — the cut assembler reassigns `effect.duration` AFTER the Filmstrip is constructed, so a frozen count drifted to n×28 slots). `#width_of_frame` is derived from the live `frames_count` (= effect_width/frames_count), and `#get_filmstrip_frame_at` divides by the same, so the view's render slot (`effect_width/frames_count`) and the draw slot BOTH stay ≈28px for every clip regardless of post-init duration changes; visible-window draw steps by `#width_of_frame` (was `/100`). Frames are still un-stretched (`effect-styles.patch.ts` pins `.thumbnail { width:28px !important; object-fit:cover }`). Horizontal scroll keeps frames aligned. Confirm: adjacent `<img>`s butt together (translateX deltas ≈ 28px across clips of DIFFERENT widths), no gap. Not-yet-drawn cells show a CLEAN dark slot, never a broken-image icon: placeholders are a baked 28×50 canvas data-URL (`#14141f` ≈ raised lane), NOT `${location.href}/assets/loading.svg` (which 404s under hash routing). Confirm no `<img naturalWidth=0>` / broken icons; visible cells fill with frames (the refreshFilmstrips re-publish runs a full visible recalc per clip after extraction). |
