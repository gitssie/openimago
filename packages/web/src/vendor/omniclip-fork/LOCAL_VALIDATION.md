...
| 10 | **Filmstrip 9:16, flush-left, not stretched** (openimago-audw / fhnz) | Inspect a clip's `.filmstrip` `<img class="thumbnail">` frames (portrait shot videos) | Each frame's natural data-URL is a 28×50 9:16 CENTER-COVER crop; rendered at exactly 28px wide, anchored LEFT in its slot (faces/figures NOT horizontally stretched). The VideoEffect view sets an inline `width = slot px` with no object-fit (default `fill` stretched it ~55px → squished); the fork's `effect-styles.patch.ts` pins `.filmstrip img.thumbnail { width:28px !important; height:50px; object-fit:cover }`. Trailing slot space showing the lane bg between frames is intended. Confirm an `<img>`'s computed width is 28px, not ~55px. |
