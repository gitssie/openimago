...
| 3 | **Edit persists** | Reorder / trim / split / delete a clip (wire the editor event to `panel.persistEdit({kind,...})`) | The matching cut endpoint is called with `expectedUpdatedAt`; on success the page refetches and the change sticks. **TRIM real-time ripple (openimago-1ky8): drag a clip's RIGHT edge shorter → the following clips snap FLUSH immediately (no black gap), without a refresh. on-edit.commit() calls rippleLiveTrack after a trim — recomputes the no-gap layout (rippleStartPositions, same cursorMs placement as hydrate / bd-4rdj invariant) and writes changed `start_at_position`s via omniclip's `set_effect_start_position`; baseline+lastSeen re-sync so the local reposition is NOT re-persisted (no spurious reorder write). Confirm: no lingering black/empty track between clips after trimming a right edge; total timeline shortens; no extra cut write fired for the ripple. RAPID double-split (openimago-vx2t): split a clip twice within ~150ms → BOTH new clips persist (refresh keeps all segments, a later reorder does NOT 400). WRITE-FAILURE surfacing (openimago-vx2t/1ky8): a non-409 write (e.g. 400) shows a visible 剪辑保存失败 error AND the editor re-hydrates to the server's truth (no silent desync).** |
