...
| 10 | **Filmstrip is 9:16, not squished** (openimago-audw) | Inspect a clip's `.filmstrip` `<img>` frames (portrait shot videos) | Each frame is a 9:16 PORTRAIT crop (32×56 canvas), CENTER-COVER — faces/figures keep real proportions (NOT horizontally stretched). Swapped omniclip's `Filmstrip` via `quasar.config.ts` `omniclipFilmstripPatch` → `patches/filmstrip.patch.ts` (9-arg drawImage crop); density/tiling unchanged from upstream. NOTE: per-clip CSS polish (selected cyan ring / imago border tokens / `--imago-bg-raised` loading fill) is NOT applied — omniclip's clip `.effect`/`.filmstrip` styles live in its SEALED shadow DOM with hardcoded hex and no `part=` on `.effect`, so host CSS / `--omni-*` can't reach them without a runtime shadow-root stylesheet injection (deferred). The clip already has upstream `border-radius:5px; overflow:hidden` + `data-selected`, so frames are corner-clipped. |
