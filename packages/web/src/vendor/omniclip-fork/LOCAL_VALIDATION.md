...
| 12 | **Filmstrip = fixed-width 9:16 first-frame tiles** (openimago-ugli → openimago-u3qq) | Open 时间线; inspect a clip's `.filmstrip` `.sprite-cell` divs (each width=28px) | SUPERSEDES row 10's px5g mapping AND the interim one-cell-per-second flex layout. The strip is a "which video is this" marker: every tile shows the video's FIRST frame (frame 0) at NATIVE 9:16 aspect — NO time→frame mapping, NO per-cell second labels. Tiles are FIXED `FILMSTRIP_TILE_W×CELL_H` = 28×50 (`flex:0 0 auto`); the lane (`overflow:hidden`) seamlessly fills the effect width and clips the trailing tile. tileCount = `ceil(effectWidthPx / 28)` clamped to MAX_CELLS, where `effectWidthPx = (effect.end - effect.start) * 2^state.zoom` (the upstream `calculate_effect_width` formula — read `context.state.zoom` directly, never subscribe the editor). Each tile crops to frame 0 by PERCENTAGE (`spriteBackgroundSizeX` = `frameCount*100%`, `background-position-x:0`); tile and frame are both 9:16 → NO distortion (the interim version stretched one frame across a wide flex cell → horizontal bar). Confirm: tiles are crisp 9:16 portrait, identical (first) frame within a clip, seamless edge-to-edge, no gaps/overflow; different-video clips show different first frames; NO sprited clip renders 0 tiles. Top SECONDS come from omniclip's own `TimeRuler` (`views/time-ruler`, unpatched; same `2^zoom` scale + `timeline` scroll origin) — we add NO duplicate labels; `applyImagoTheme` only sets `--imago-*` color tokens, it does not hide/move the ruler. The dead `.filmstrip img.thumbnail` rule remains removed. |
