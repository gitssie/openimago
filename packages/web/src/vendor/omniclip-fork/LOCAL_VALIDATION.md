...
| 3 | **Edit persists** | Reorder / trim / split / delete a clip (wire the editor event to `panel.persistEdit({kind,...})`) | The matching cut endpoint is called with `expectedUpdatedAt`; on success the page refetches and the change sticks. **Real-time no-gap ripple after TRIM / REORDER / DELETE (openimago-1ky8, extended openimago-uvm4): trim a clip's RIGHT edge shorter, OR move(reorder) a clip, OR delete a clip → the following clips snap FLUSH immediately (no black gap), without a refresh. on-edit.commit() sets a `sawLayoutChange` flag for trim/reorder/delete (a split's two halves already abut, so it is excluded) and calls rippleLiveTrack — recomputes the no-gap layout (rippleStartPositions, same cursorMs placement as hydrate / bd-4rdj invariant) and writes changed `start_at_position`s via omniclip's `set_effect_start_position`; baseline+lastSeen re-sync so the local reposition is NOT re-persisted (no spurious reorder write, no conflict with the vx2t split drain). Confirm: no lingering black/empty track between clips after a trim, a move, or a delete; total timeline closes the gap; the persisted order/segment-count is exact after refresh; no extra cut write fired for the ripple itself. RAPID double-split (openimago-vx2t): split a clip twice within ~150ms → BOTH new clips persist (refresh keeps all segments, a later reorder does NOT 400). WRITE-FAILURE surfacing (openimago-vx2t/1ky8): a non-409 write (e.g. 400) shows a visible 剪辑保存失败 error AND the editor re-hydrates to the server's truth (no silent desync).** |
...
| 10 | **Filmstrip = static sprite sheet (no WebCodecs)** (openimago-78m9) | Open 时间线, assemble/hydrate; inspect a clip's `.filmstrip` — it now contains `.sprite-cell` `<div>`s (NOT `<img class="thumbnail">`); scroll the timeline | The timeline filmstrip is rendered STATICALLY from a precomputed sprite sheet (`public/mock/<name>.filmstrip.png`, one horizontal strip of 24 9:16 frames) via CSS `background-image` + `background-position` — NO client-side decode/seek. Result: INSTANT, smooth, NO lag, NO flicker, NO white/broken frames, NO appear/disappear on scroll. **The exact tile LAYOUT/frame-selection is the FIRST-FRAME tiling in row 12 (openimago-jmcp), which is the FINAL behavior — see there.** (Historical note: an earlier px5g revision mapped each cell to a per-second source frame offset by the clip's `inPoint` via `filmstrip_duration_seconds`/`filmstrip_source_duration_seconds`; that made a split's SECOND half offset its sprite window past the strip end → **blank/black**, and was reverted to first-frame anchoring per the `cut-editor-filmstrip-firstframe` decision. Those px5g effect fields are no longer threaded by `hydrate-from-cut.ts` and no longer read by the view — openimago-hkhm.) Sprite URL+dims come from the run's `result.access.filmstrip` (+`result.filmstrip{frameCount,frameW,frameH}`) → `StoryRunSummary.filmstripUrl` → shot-media-resolver → `HydrateClip.filmstrip` → the effect's `filmstrip_url`, read by `patches/video-effect.patch.ts`. Swapped via `omniclipVideoEffectPatch` (resolveId on `views/effects/video-effect.js`). Confirm: cells are sprite-backed divs, scrolling causes ZERO re-decode/flicker; the preview PLAYER still plays via WebCodecs (unchanged). |
...
| 12 | **Filmstrip = fixed-width 9:16 first-frame tiles, tiled to fill width** (openimago-ugli → u3qq → 7vrd → **openimago-jmcp final**) | Open 时间线; inspect a clip's `.filmstrip` → `.sprite-cell` divs (each width=28px) filling the clip edge-to-edge | FINAL layout (re-adopts u3qq's continuous tiling; supersedes 7vrd's one-per-second sparse version — user confirmed they want the lane CONTINUOUSLY filled, density following zoom). The strip is a "which video is this" marker: every tile shows the video's FIRST frame (frame 0) at NATIVE 9:16 aspect — NO time→frame mapping, NO per-cell second labels. Tiles are FIXED `FILMSTRIP_TILE_W×CELL_H` = 28×50 (`flex:0 0 auto`); `tileCount = ceil(effectWidthPx / 28)` clamped to MAX_CELLS, where `effectWidthPx = (effect.end - effect.start) * 2^state.zoom` (the upstream `calculate_effect_width` formula — read `context.state.zoom` directly, never subscribe the editor). The lane (`overflow:hidden`) seamlessly fills the effect width and clips the trailing tile. Each tile crops to frame 0 by PERCENTAGE (`spriteBackgroundSizeX` = `frameCount*100%`, `background-position-x:0`); tile and frame are both 9:16 → NO distortion (the ugli version stretched one frame across a wide cell → horizontal bar). Confirm: tiles are crisp 9:16 portrait, identical (first) frame within a clip, seamless edge-to-edge with no gaps/overflow; density grows with zoom; long clips (s07 64s) tile many (wide, scrollable); different-video clips show different first frames; zero-width/orphan → empty lane, very short clip → ≥1 tile; NO sprited clip renders 0. **SPLIT regression check (openimago-hkhm): select a clip, split it mid-way, then click the SECOND (right) half — its filmstrip MUST show the same source first-frame sprite tiles as the first half, NOT blank/black. Both halves share the source `file_hash`; the view resolves the sprite from `live` and, if a native split's new half lacks it, falls back to any sibling effect with the same `file_hash` that has a sprite. Repeat the split on a non-first clip (s02+, whose `start_at_position` ≠ source `start`) — still no blank.** Top SECONDS come from omniclip's own `TimeRuler` (`views/time-ruler`, unpatched; same `2^zoom` scale + `timeline` scroll origin) — we add NO duplicate labels; `applyImagoTheme` only sets `--imago-*` color tokens, it does not hide/move the ruler. The dead `.filmstrip img.thumbnail` rule remains removed; the 7vrd `filmstripCellCount` (one-per-second) was removed. |
| 13 | **Preview player: white playhead + two-panel layout + portrait 9:16 + first frame paints** (openimago-h9pt/h8v6/rgtw/**vm5v**) | Open 时间线 tab; look at the editor ABOVE the timeline | (h8v6) The editor shows a vertical TWO-pane layout — a **video preview player on top (~62%)** + timeline below — rebuilt in `index.ts` via `omnislate.context = new OmniContext({panels:{TimelinePanel,MediaPlayerPanel}, layouts: two-pane})` + `layout.reset_to_default()`; `.story-cut__construct{display:block;height:100%}` lets `<construct-editor>` fill the host. (h9pt) Playhead is a WHITE thin line (not omniclip yellow) via `media-player`-sibling `playhead-styles.patch.ts`. (vm5v) The preview canvas is **portrait 9:16** (1080×1920), centered in the panel with acceptable dark margins — set at boot via `actions.set_project_resolution(1080,1920)` + `compositor.set_canvas_resolution(1080,1920)`, `fullFrameRect` → 1080×1920 (hydrate-from-cut + cut-omniclip-mapper), and `media-player-styles.patch.ts` overrides `figure`/`.canvas-container` to `aspect-ratio:9/16` (canvas already `width/height:100%!important` upstream). (rgtw) The FIRST frame paints on open (not black): `composePlacedClips` → `nudgeFirstFrame` waits for each on-screen clip's `<video>` to reach a drawable readyState then re-runs `compose_effects`+redraw. (openimago-6imt) The wait is now EVENT-DRIVEN: `nudgeFirstFrame` arms `loadeddata`/`canplay`/`seeked` listeners that fire a repaint whenever a clip becomes drawable — re-attaching each rAF tick to clips that appear AFTER the nudge starts (a user split's new clip is built asynchronously by `recreate`) — and the rAF loop is bounded by a generous 15s TIME budget (was a tight ~3s/180-frame cap) so a slow decode is no longer abandoned with a black frame; the listeners outlive the loop and detach once their clip is drawable+on-canvas. After a SPLIT, `on-edit.commit()` calls `nudgeFirstFrame` (deferred 2 rAFs) so the freshly-created half repaints. Confirm: portrait canvas centered showing the first frame; ▶ plays & canvas updates; drag playhead seeks; NO landscape letterbox / exposed `.lower-canvas`. **SPLIT preview check (openimago-6imt): split a clip, select the SECOND (right) half → the PREVIEW CANVAS shows that segment's first frame within a few seconds, NOT black/empty; repeat on a non-first clip (large inPoint). A DEV `[omni-diag] nudgeFirstFrame rAF budget elapsed` log is acceptable IF the frame still paints — the media-ready listeners repaint on the late decode.** Three styles.js patches (effect/playhead/media-player) are gated on distinct importers (`effects/parts/effect.js` / `views/playhead/view.js` / `views/media-player/view.js`) so they never cross-fire. Real-browser only (headless Chromium may lack H.264 → black is a codec limit, not a regression). |
