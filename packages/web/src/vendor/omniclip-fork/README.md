...
- `patches/video-effect.patch.ts` — drop-in `VideoEffect` timeline-view replacement
  (openimago-78m9): renders the filmstrip as a STATIC precomputed sprite sheet via
  CSS `background-position` (a row of 28×50 9:16 cells, each pointing at one sprite
  frame) — NO client-side WebCodecs frame extraction (that caused lag/flicker/white
  frames). Sprite URL + dims are carried on the effect (`filmstrip_url` etc.,
  threaded by hydrate-from-cut). Still composes the clip into the WebCodecs preview
  PLAYER via the `on_media_change("added")` listener (playback unchanged). Swapped
  via `omniclipVideoEffectPatch` (`resolveId` on `views/effects/video-effect.js`).
  Replaced the former `filmstrip.patch.ts` (WebCodecs extractor — deleted).
  Sprites are pre-generated by `scripts/gen-filmstrips.mjs` into `public/mock/`.
