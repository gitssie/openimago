...
// The timeline filmstrip lane is tiled with fixed-width 9:16 thumbnails of the
// video's FIRST frame (openimago-u3qq). Each tile is a fixed 28×50 box (9:16,
// matching the 28×50 sprite frame), so the portrait frame shows at native ratio
// — NOT stretched across a wide flex cell (the openimago-ugli regression). The
// lane clips overflow, so the tiles seamlessly fill the effect's real width.
