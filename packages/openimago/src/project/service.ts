  ...
  async scaffoldProjectFiles(
    ...
    if (!(await this.fileExists(agentsPath))) {
      const agentsContent = [
        ...
        "```",
        "",
        "   For a VIDEO run, also copy the artifact's `access.filmstrip` (URL) and its",
        "   `filmstrip` dims `{ frameCount, frameW, frameH }` into the run's `result`",
        "   (`result.access.filmstrip` + `result.filmstrip`) — the backend generates the",
        "   timeline filmstrip sprite for the video artifact (openimago-k6bl); the NLE",
        "   timeline renders it. It is set asynchronously, so read it from the artifact",
        "   once available (skip if absent).",
        "",
